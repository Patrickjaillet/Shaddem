...
public sealed class ShaderProgram : IDisposable
{
    ...
    public ShaderProgram(GL gl, string name, string vertexSource, string fragmentSource, bool useBinaryCache = false)
    {
        _gl = gl;
        Name = name;
        ...
        if (useBinaryCache)
        {
            string hash = ShaderBinaryCache.ComputeHash(vertexSource, fragmentSource);
            if (ShaderBinaryCache.TryLoad(gl, hash, _handle))
            {
                return;
            }
        }
        ...
        if (useBinaryCache)
        {
            gl.ProgramParameter(_handle, GLEnum.ProgramBinaryRetrievableHint, 1);
        }
        ...
        {
            string hash = ShaderBinaryCache.ComputeHash(vertexSource, fragmentSource);
            ShaderBinaryCache.Save(gl, hash, _handle);
        }
    }
