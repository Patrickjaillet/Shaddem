...
public static class PresetsService
{
    ...
    public static Dictionary<string, EffectParams> Load(string filePath)
    {
        ...
        Dictionary<string, EffectParams> presets = JsonSerializer.Deserialize<Dictionary<string, EffectParams>>(json, JsonOptions) ?? new Dictionary<string, EffectParams>();
        foreach (EffectParams preset in presets.Values) preset.ClampToValidRanges();
        return presets;
    }
