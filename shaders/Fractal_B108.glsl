...
float de(vec3 p, out vec4 col) {
    ...
    for(int i = 0; i < 6; i++) {
        ...
        p = p * 1.75 - vec3(0.1, 0.8, 0.2);
        scale *= 1.75;
        
        float box = sdBox(p, vec3(0.4, 1.2, 0.4)) / scale;
        d = min(d, box);
        
        if(i < 4) orbit = min(orbit, abs(p));
    }
// https://github.com/Patrickjaillet/Z-GL
    col = vec4(sin(orbit * 3.5) * 0.4 + vec3(0.6), 1.0);
    return d;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(t < maxD) {
        ...
        vec3 l1 = normalize(vec3(1.5, 2.5, -2.0));
        vec3 l2 = normalize(vec3(-1.5, -1.5, -1.0));
        ...
        vec3 baseLight = objCol.rgb * (diff1 + diff2 + 0.15);
        sceneColor = baseLight + vec3(0.8) * spec + vec3(0.2, 0.4, 0.7) * fre;
        sceneColor *= ao;
        
        sceneColor = mix(sceneColor, vec3(0.005, 0.01, 0.02), 1.0 - exp(-0.1 * t * t));
    } else {
        sceneColor = vec3(0.002, 0.004, 0.008) * (1.0 - length(uv));
    }
    
    sceneColor += vec3(0.1, 0.5, 0.9) * glow * 0.012;
    sceneColor += vec3(0.8, 0.2, 0.4) * glow * 0.003 * (sin(iTime) * 0.5 + 0.5);
