...
vec2 tunnelPath(float z) {
    return vec2(
        sin(z * 0.12) * 3.5 + cos(z * 0.04) * 1.5, 
        cos(z * 0.09) * 2.5 + sin(z * 0.07) * 1.2
    );
}
...
float map(vec3 p) {
    ...
    p2.xy *= mm2(p.z * 0.1);
    float vines = length(p2.xy + vec2(sin(p.z), cos(p.z)) * 1.5) - 0.8;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {	
    ...
    if (t < 50.0) {
        ...
        col = mix(vec3(0.05, 0.1, 0.25), vec3(0.4, 0.6, 0.8), n);
    ...
    fragColor = vec4(col * smoothstep(0.0, 2.5, iTime), 1.0);
}